...
namespace MyHotel.Api.Controllers
{
    ...
    public class InvitesController : ControllerBase
    {
        ...
        public async Task<ActionResult> Post([FromBody] InvitePostModel i)
        {
            ...
            foreach (var item in i.RoomIds)
            {
                rooms.Add(await _roomService.GetRoomByIdAsync(item));
            }
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult> Put(int id, [FromBody] InvitePostModel i)
        {
            //Invite i1 = dataContext.Invites.Find(x => x.CodeInvite == id);
            //if (i1 == null)
            //    return NotFound();
            Invite i1 =await _inviteService.GetInviteByIdAsync(id);
            _mapper.Map(i, i1);
            await _inviteService.UpdateInviteAsync(id, i1);
            return Ok();
        }
